...
func main() {
	...
	for {
		...
		fmt.Println("2. Login sebagai Mahasiswa")
...
func kerjakanTugas(reader *bufio.Reader, student *User) {
	...
	for i, task := range lmsData.Tasks {
		if task.ID == taskID {
			...
			if answer != "" {
				lmsData.Tasks[i].Submission = answer // Menggunakan indeks untuk memperbarui array
				fmt.Println("Jawaban berhasil dikumpulkan!")
			} else {
				fmt.Println("Jawaban tidak boleh kosong.")
				return
			}
...
func kerjakanQuiz(reader *bufio.Reader, student *User) {
	...
	for _, quiz := range lmsData.Quizzes {
		if quiz.ID == quizID {
			...
			// Array untuk menyimpan semua jawaban siswa
			...
			for i, question := range quiz.Questions {
				...
				answer, _ := reader.ReadString('\n')
				answer = strings.TrimSpace(answer)
				allAnswers = append(allAnswers, answer) // Simpan jawaban ke dalam array
				fmt.Println()
			}

			fmt.Println("Quiz selesai dikerjakan!")
			fmt.Println("Loading......")
			time.Sleep(3 * time.Second)
			...
			studentAnswers = append(studentAnswers, StudentAnswer{
				...
				TaskID:      quizID,
				StudentName: student.Username,
				Answer:      strings.Join(allAnswers, "; "), // Gabungkan semua jawaban dengan delimiter
				Graded:      false,
				Grade:       "",
			})
			nextAnswerID++
...
func ikutiForum(reader *bufio.Reader) {
	...
	for i, forum := range lmsData.Forums {
		if forum.ID == forumID {
			...
			fmt.Print("Masukkan pendapat atau jawaban Anda: ")
			newPost, _ := reader.ReadString('\n')
			newPost = strings.TrimSpace(newPost)

			if newPost != "" {
				lmsData.Forums[i].Posts = append(lmsData.Forums[i].Posts, newPost)
				fmt.Println("Pendapat Anda berhasil ditambahkan ke forum.")
			} else {
				fmt.Println("Pendapat tidak boleh kosong.")
			}
